...
if($_POST["action"]=="listarChat"){
    $arrayChats=[];
    $result=$oficina->listarChats($_SESSION["id_user"]);
    ...
    for($i=0;$i<sizeof($data);$i++){
        ...
        if($nombreChat==""){
            $result=$oficina->buscarUsuariosChat($idChat);
            ...
            $result=$oficina->buscarNombreUsuarioChat($usr);
        ...
        $result=$oficina->buscarUltimoMensaje($idChat);
        if($result!=null){
            $text=$result["text"];
        ...
        $result=$oficina->buscarMensajesPendientes($idChat,$_SESSION["id_user"]);
        $nMensajes=0;
        if($result!=null){
...
if($_POST["action"]=="chatData2"){
    $result=$oficina->buscarDatosChat($_POST["id_chat"]);

    $nombre=$result["Nombre"];
    $permisos=-1;
    $avatar=$result["avatar"];
    $result2=$oficina->buscarUsuariosChat($_POST["id_chat"]);
    $array = explode(",", $result["id_usuarios"]);
    foreach ($array as $usuario){
        if($usuario!=$_SESSION['id_user'] && $usuario!=""){
            $usr=$usuario;
    ...
    if($nombre==NULL){
        $result=$oficina->buscarNombreUsuarioChat($_POST["id_chat"],$_SESSION["id_user"]);
        $result=$oficina->buscarNombreUsuarioChat($usr);
        $nombre=$result["nombre"];
        $permisos=$result["tipo_usuario"];
        $avatar=$result["avatar"];
...
if($_POST["action"]=="chatData3"){
    $result=$oficina->buscarNombreUsuarioChat($_POST["id_user"]);
     $nombre=$result["nombre"]; 
     $permisos=$result["tipo_usuario"]; 
     $avatar=$result["avatar"];
