...
//FUNCION QUE LISTA LOS CHATS DEL USUARIO
    ...
    if(arrayChats.length>0){
      ...
      for(let i=0;i<arrayChats.length;i++){
        ...
        if(arrayChats[i][9]=="" || arrayChats[i][9]==null){
          //console.log(arrayChats[i][4]+" "+fecha)
          if(arrayChats[i][7]=="GRUPO"){
            HTMLcard+='<div class="icon-image" style="background-color:green">'+arrayChats[i][1].charAt(0)+'</div>'+
  ...
  function init(){
    ...
    conn.onmessage = function (e) {
        ...
        if(respuesta.idUsers!=null && respuesta.idUsers.includes(sessionId.toString())){
            recargar();
         }
    }
