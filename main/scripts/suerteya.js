$(document).ready(function(){
  let nMensajesSuerteyaTotales=-1;
  let ofiName=document.querySelector("#suerteya-tab")
  let ofiNameOriginal=ofiName.innerHTML;
   let arrayChats=new Array();
  
   let ultimaCargaDatosSuert;
...
//FUNCION QUE LISTA LOS CHATS DEL USUARIO
  function listarChat() {
    ...
    //   if(ultimaCargaDatosSuert!=data){
    //     ultimaCargaDatosSuert=data
     data=JSON.parse(data);
      ...
      if( document.querySelector("#chatOficina").innerHTML=""){
  ...
  arrayChats=arrayText.concat(arrayNoText)
    ...
    if(arrayChats.length>0){
      ...
      for(let i=0;i<arrayChats.length;i++){
        ...
        let HTMLcard='<div class="chat-card bg-white" id="'+arrayChats[i][0]+'" msg="'+nMensajes+'">'
      ...
      if(nMensajesSuerteyaTotales==-1){
        nMensajesSuerteyaTotales=nMensajesOficina;
      }
      
      if(nMensajesSuerteyaTotales<nMensajesOficina){
        mandarNotificacion("Ha recibido un mensaje");
        nMensajesSuerteyaTotales=nMensajesOficina;
      }
  ...
  function init(){
    ...
    conn.onmessage = function (e) {
        ...
        if(respuesta.idUsers.includes(sessionId)){
            recargar();
         }
    }
