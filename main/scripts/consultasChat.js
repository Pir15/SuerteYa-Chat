$(document).ready(function(){
  ...
  function datosChat(idChat){
      ...
      $.post( "controller/consultas.php",{action:"datosChat",id_chat:idChat},
    function(data) {
      $('#divLoad').modal("hide");
      //console.log(data);
    ...
    //   ultimoEstado=data[4];
      ...
      // }else{
        chatLogo.textContent = data[0].charAt(0);
        chatLogo.className="icon-image-mini";
      // }
  ...
  //   $.post( "controller/consultas.php",{action:"estadoChat",id_chat:chat},
  //   function(data) {
  ...
  function mensajes(idChat){
    if(idChat!=0){
    $.post( "controller/consultas.php",{action:"mensajes",idChat:idChat,nMensajes:nMensajes},
    function(data) {
      //console.log(data);
      ...
      for(let mensaje of data){
        ...
        if(mensaje[4]){
          ...
          divMensaje.innerHTML="<div class='mensajeVerde' style='max-width:"+window.innerWidth/1.5+"px;'><b>"+mensaje[2]+"</b><i class='date-label'> "+vistaFecha(mensaje[3])+"</i><br><span class='txt'>"+mensaje[1]+"</span></div>";
        }else{
          divMensaje.className="div-mensajeRight";
          divMensaje.innerHTML="<div class='mensajeBlanco' style='max-width:"+window.innerWidth/1.5+"px;'><b>"+mensaje[2]+"</b><i class='date-label'> "+vistaFecha(mensaje[3])+"</i><br><span class='txt'>"+mensaje[1]+"</span></div>";
...
//FUNCION QUE ENVIA MENSAJES AL CHAT
  function mandarMensajes(idChat){
      ...
      if(txt.length>=150){
        let car=txt.length;
        $.post( "controller/consultas.php",{action:"insertarConsulta",idChat:idChat,txt:txt,car:car},
        function(data) {
         console.log(data)
        });
        mensajes(idChat);
  ...
  //   $.post( "controller/consultas.php",{action:"getSaldo"},
  //   function(data) {
  ...
  function mensajeLeido(array){
     $.post( "controller/consultas.php",{action:"setLeido",mensajes:array},function(data){
      //console.log(data);
     });  
  }
  ...
  document.querySelector("#textInput").addEventListener("keyup",function(e){
    ...
    // }else{
      if(length<150){
        document.querySelector("#nCaracteres").style.color="red";
      }else{
        document.querySelector("#nCaracteres").style.color="green";
      }
      document.querySelector("#nCaracteres").textContent=length+"/150"
    //}
