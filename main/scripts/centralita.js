$(document).ready(function(){
  let nMensajesCentralitaTotales=-1;
  let centName=document.querySelector("#centralita-tab")
  let centNameOriginal=centName.innerHTML;

  let arrayChats=new Array();

  let ultimaCargaDatosCent;
...
//FUNCION QUE LISTA LOS CHATS DEL USUARIO
 function listarChat() {
   $.post( "controller/centralita.php",{action:"listarChat"},
   function(data) {
    //    console.log(data)
    //  if(ultimaCargaDatosCent!=data){
    //   ultimaCargaDatosCent=data
     data=JSON.parse(data);
     if(data.length>0){
      arrayChats.length=0;
     for(let i=0;i<data.length;i++) {
     
       let chatObjetCentralita=new Array(data[i][0],data[i][2],data[i][4],data[i][3],data[i][5],data[i][6],data[i][7],data[i][1],data[i][8],data[i][9]);
       arrayChats.push(chatObjetCentralita);
     }
   }else{
...
arrayChats=arrayText.concat(arrayNoText)
  ...
  let nMensajesCentralita=0;

  if(arrayChats.length>0){
    quitarScrollCarga();
    document.querySelector("#chatCentralita").innerHTML="";
    ...
    for(let i=0;i<arrayChats.length;i++){
      ...
      chatContainer.addEventListener("click",function(){
        window.history.replaceState("", "", "index.php?lsc="+19);
        window.open("chat.php?idChat="+btoa(id)+"&msg="+nMensajes+"&rusr="+19,"_self");
      })
      ...
      if(nMensajes>0){
        ...
        nMensajesCentralita+=arrayChats[i][5]
    ...
    document.querySelector("#chatCentralita").appendChild(chatContainer);
   
    
    }
    
    if(nMensajesCentralita>0){
      centName.innerHTML=centNameOriginal+" ("+nMensajesCentralita+")";
    }else{
      centName.innerHTML=centNameOriginal
    }

    if(nMensajesCentralitaTotales==-1){
      nMensajesCentralitaTotales=nMensajesCentralita;
    }
    
    if(nMensajesCentralitaTotales<nMensajesCentralita){
      mandarNotificacion("Ha recibido un mensaje en CENTRALITA");
      nMensajesCentralitaTotales=nMensajesCentralita;
    }
 ...
 function init(){
    ...
    conn.onmessage = function (e) {
        ...
        if(respuesta.idUsers!=null && respuesta.idUsers.includes('19,')){
            recargar();
         }
    }
