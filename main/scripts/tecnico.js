$(document).ready(function(){
  ...
  let nMensajesTecnicoTotales=-1;
  let tecnName=document.querySelector("#tecnico-tab")
  let tecnNameOriginal=tecnName.innerHTML;
  let arrayChats=new Array();

  let ultimaCargaDatosTecnico;
...
//FUNCION QUE LISTA LOS CHATS DEL USUARIO
 function listarChat() {
   $.post( "controller/tecnico.php",{action:"listarChat"},
   function(data) {
    
    // if(ultimaCargaDatosTecnico!=data){
    //   ultimaCargaDatosTecnico=data
  ...
  if(arrayChats.length>0){
    ...
    for(let i=0;i<arrayChats.length;i++){
      ...
      chatContainer.addEventListener("click",function(){
        let url="chat.php?idChat="+btoa(id)+"&rusr="+21+"&msg="+nMensajes;
        window.history.replaceState("", "", "index.php?lsc="+21);
      ...
      if(nMensajes>0){
        ...
        nMensajesTecnico+=arrayChats[i][5]
    ...
    document.querySelector("#chatTecnico").appendChild(chatContainer);
   
    
    }
    if(nMensajesTecnico>0){
      tecnName.innerHTML=tecnNameOriginal+" ("+nMensajesTecnico+")";
    }else{
      tecnName.innerHTML=tecnNameOriginal
    }

    if(nMensajesTecnicoTotales==-1){
      nMensajesTecnicoTotales=nMensajesTecnico;
    }
    
    if(nMensajesTecnicoTotales<nMensajesTecnico){
      mandarNotificacion("Ha recibido un mensaje en TECNICO");
      nMensajesTecnicoTotales=nMensajesTecnico;
    }
...
//  //FUNCION QUE CARGA EL CODIGO A EJECUTAR EN EL INICIO
 ...
 function init(){
    ...
    conn.onmessage = function (e) {
        ...
        if(respuesta.idUsers!=null && respuesta.idUsers.includes('21,')){
            recargar();
         }
    }
